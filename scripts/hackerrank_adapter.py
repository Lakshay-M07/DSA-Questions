...
class HackerRankClient:
    """Read-only HackerRank Community client using the currently verified REST flow."""
    ...
    def authenticate(self) -> None:
        ...
        if not csrf:
            match = re.search(
                r'csrf[_-]?token["\']?\s*[:=]\s*["\']([^"\']+)', page.text, re.I
            )
        ...
        try:
            ...
            raise RuntimeError("HackerRank authentication did not return JSON") from exc

        returned_csrf = payload.get("csrf_token")
        if returned_csrf:
            self.session.headers["x-csrf-token"] = returned_csrf
    ...
    def _get(self, url: str, **params: Any) -> Any:
        ...
        if response.status_code != 200:
            raise RuntimeError(f"HackerRank REST request failed: HTTP {response.status_code} {response.url}")
        try:
            return response.json()
        except ValueError as exc:
            raise RuntimeError(f"HackerRank REST endpoint returned non-JSON data: {response.url}") from exc
    ...
    def fetch_submissions(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        if not isinstance(models, list):
            raise RuntimeError("HackerRank submissions response does not contain a models list")
        return [x for x in models if isinstance(x, dict)]

    def fetch_submission_source(self, slug: str, submission_id: str) -> dict[str, Any]:
        # This endpoint was historically used for individual submissions; it is
        # called only after the authenticated current submissions endpoint succeeds.
...
def normalize_language(value: Any) -> tuple[str, str]:
    ...
    if "ruby" in text:
        return "Ruby", ".rb"
    if "go" == text or text.startswith("go "):
        return "Go", ".go"
    if "rust" in text:
        return "Rust", ".rs"
...
def extract_source(data: Any) -> str | None:
    """Find source code without assuming one fixed historical response schema."""
...
def extract_problem_metadata(data: dict[str, Any]) -> tuple[str | None, str | None, tuple[str, ...], str | None]:
    ...
    if isinstance(difficulty, dict):
        difficulty = first_value(difficulty, "name", "label", "value", "level")
    difficulty = DIFFICULTY_WORDS.get(str(difficulty).strip().lower(), difficulty) if difficulty else None
    if isinstance(category, dict):
        category = first_value(category, "name", "label", "slug")
...
def parse_submission(record: dict[str, Any]) -> HackerRankSubmission | None:
    ...
    if submission_id is None or slug is None:
        return None
    ...
    return HackerRankSubmission(
        submission_id=str(submission_id),
        problem_id=str(first_value(record, "challenge_id", "challengeId", "id") or slug),
        slug=str(slug),
        title=str(title or slug.replace("-", " ").title()),
        language=language,
        extension=extension,
        status="Accepted",
