...
def build_readme(problem: HackerRankSubmission, difficulty: str, category: str, tags: tuple[str, ...], description: str | None) -> str:
    lines = [
        f"# {problem.title}", "", f"- **Platform:** HackerRank", f"- **Question ID:** {problem.problem_id}",
        f"- **Difficulty:** {difficulty}", f"- **Category:** {category}", f"- **Language:** {problem.language}",
    ]
    if tags: lines.append(f"- **Tags:** {', '.join(tags)}")
    if problem.submitted_at: lines.append(f"- **Submitted:** {problem.submitted_at}")
    lines += ["", "## Question", "", description or "Problem description was not exposed by the current HackerRank endpoint.", "", "## Solution", "", f"See `{Path(problem.title).name}{problem.extension}` in this directory.", ""]
    return "\n".join(lines)
...
def import_one(client: HackerRankClient, submission: HackerRankSubmission, state: set[str]) -> bool:
    ...
    if not difficulty:
        raise RuntimeError(f"No authoritative/classifiable difficulty found for HackerRank problem {submission.slug}; refusing to guess")
    if title: submission = HackerRankSubmission(**{**submission.__dict__, "title": title})
    category = category or category_from_metadata(challenge)
    folder = ROOT / "HackerRank" / submission.language / slugify(category) / difficulty / f"{submission.problem_id}-{slugify(submission.title)}"
    folder.mkdir(parents=True, exist_ok=True)
    source_path = folder / f"{submission.problem_id}-{slugify(submission.title)}{submission.extension}"
    ...
    git("commit", "Add HackerRank: " + submission.title + " (" + submission.language + ")")
    git("push")
    return True
...
def main() -> None:
    ...
    for record in records:
        ...
        if parsed: accepted.append(parsed)
    accepted.sort(key=lambda x: x.submitted_at or "")
    imported = 0
    for submission in accepted:
        if import_one(client, submission, state): imported += 1
